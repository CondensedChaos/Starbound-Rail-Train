...
function init()
  ...
  storage.disabled = object.getInputNodeLevel(0)
  if storage.disabled == true then
...
function update(dt)
  if (not world.regionActive(storage.chunk)) and (not storage.disabled) then
	world.loadRegion(storage.chunk )
  end
  if not storage.disabled then
    world.loadRegion(storage.chunk)
  end
end
